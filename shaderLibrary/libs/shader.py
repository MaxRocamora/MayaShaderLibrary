...
class Shader(object):
    def __init__(self, idName, category):
        self.properties = {}
        self._idName = idName
...
# --------------------------------------------------------------------------------------------
    ...
    def save(self):
        ...
        print 'Adding this shader {} into this category {}'.format(self.name, self.category)
    ...
    def saveShaderProperties(self):
        ...
        with open(self.configFile, 'r') as loadedJsn:
            ...
            dictData["category"] = self.category
...
# --------------------------------------------------------------------------------------------
    ...
    @staticmethod
    def loadShader(idName, category):
        '''
        Use this method to load a shader from disk.
        Args:
            idName (string): name of the internal folder of this shader
        Returns:
            shader class object
        '''
        shader = Shader(idName, category)
        shader.loadShaderProperties()
        return shader

    @staticmethod
    def createShader(shaderData):
        '''
        Use this method to create a virtual shader, sets own attributes and returns this class
        the id is generated by taking the last free folder from 'SHD_0000' pattern
        Args:
        ...
        while 'SHD_' + str(nextId).zfill(4) in os.listdir(categoryPath):
            nextId += 1
        idName = 'SHD_' + str(nextId).zfill(4)
        shader = Shader(idName, shaderData['category'])
    ...
    def getShader(category):
        ...
        if shaders[0] in ignoreDefaults:
            return False, 'Shader found is maya default ({})'.format(shaders[0])
